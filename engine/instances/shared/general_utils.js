...
class LightingLayer extends EngineInstance {
	onCreate() {
		...
		this.pixels = [];
		this.pixelRenderTexture = $engine.createManagedRenderable(
			this,
			PIXI.RenderTexture.create($engine.getWindowSizeX(), $engine.getWindowSizeY())
		);
		...
		this.lightRenderTexture = $engine.createManagedRenderable(
			this,
			PIXI.RenderTexture.create($engine.getWindowSizeX() + 256, $engine.getWindowSizeY() + 256)
		);
	...
	raytraceFrom(startX, startY, numPoints = -1, maxSteps = -1, pixelsPerStep = -1) {
		...
		if (numPoints === -1) numPoints = this.numPoints;
		if (maxSteps === -1) maxSteps = this.maxSteps;
		if (pixelsPerStep === -1) pixelsPerStep = this.pixelsPerStep;
		...
		for (var i = 0; i < numPoints; i++) {
			...
			for (var t = 0; t < maxSteps; t++) {
				...
				if (this.pixels[(cxf + cyf * 816) << 2] !== 0xff) {
					// since it's black and white, we only need to check one
...
class AnimatedParticle extends EngineInstance {
	...
	step() {
		if (!this.paused) this.anim.update(1);
	}
...
class ExplosionParticle extends EngineInstance {
	...
	step() {
		if (this.animated) this.getSprite().update(1);

		this.lifeTimer++;
		if (this.lifeTimer > this.lifeTime) this.destroy();
		this.alpha = EngineUtils.interpolate(
			(this.lifeTimer - (this.lifeTime - 24)) / 24,
			1,
			0,
