...
class InstanceMover extends EngineInstance {
	...
	__groundControl(accelVector, velocity) {
		for (var i = 0; i < 2; ++i) {
			if (this.turnLagTimer[i] >= 0) {
				...
				if (this.turnLagTimer[i] != 0) velocity[i] -= change;
				else {
					velocity[i] = 0;
					//if(vel[(i+1)%2]==0)
			...
			if (velocity[i] != 0 && Math.sign(accelVector[i]) != Math.sign(velocity[i])) {
				if (accelVector[i] !== 0 && Math.abs(accelVector[i]) > Math.abs(velocity[i])) {
					// allow the player to break
		...
		for (var i = 0; i < 2; ++i) {
			if (Math.abs(velocity[i]) > maxSpd) {
				...
				Math.sign(accelVector[i]) === Math.sign(velocity[i]) &&
				Math.abs(accelVector[i] + velocity[i]) > maxSpd
			)
	...
	__applyDrag() {
		for (var i = 0; i < 2; ++i) {
			...
			if (Math.abs(this.vel[i]) < this.drag) this.vel[i] = 0;
		}
	}
	...
	__snapX() {
		...
		if (this.vel[0] > 0 || !this.allowSnapLeft)
			// cannot snap in a direction you are not moving...
			lowerBound = 0;
		if (this.vel[0] < 0 || !this.allowSnapRight) upperBound = 0;
	...
	__snapY() {
		...
		if (this.vel[1] > 0 || !this.allowSnapUp)
			// cannot snap in a direction you are not moving...
			lowerBound = 0;
		if (this.vel[1] < 0 || !this.allowSnapDown) upperBound = 0;
		for (var i = 0; i < upperBound; i++) {
