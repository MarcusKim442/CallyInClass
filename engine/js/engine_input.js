class IN {
	static __register() {
		...
		document.addEventListener("keydown", (e) => {
			...
			if (!e.repeat && IN.__heldKeys.indexOf(e.code) === -1 && IN.__pressedKeysCarry.indexOf(e.code) === -1) {
				...
				if (e.key !== e.code)
					// also allow the programmer to state the meaning of a key
				...
				if (IN.__debugRecordKeyPress) console.log(e.code, " :: ", e.key, "::", rpgKey);
			}
			e.__handled = true;
		});
		...
		document.addEventListener("keyup", (e) => {
			...
			if (e.key !== e.code) IN.__releasedKeysCarry.push(e.key);
			var rpgKey = Input.keyMapper[e.keyCode];
			if (rpgKey) IN.__releasedKeysCarry.push("RPG" + rpgKey);
			e.__handled = true;
		});
	...
	static __onMouseEvent(event) {
		...
		if (IN.__debugRecordMousePress) console.log(type + " :: " + event.button);
	...
	static __forceClear() {
		// forces IN to clear it's known keys. useful for when the game starts
		IN.__heldKeys = [];
		IN.__releasedKeysCarry = [];
		IN.__pressedKeysCarry = [];
