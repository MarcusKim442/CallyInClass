...
class EngineUtils {
	...
	/**@deprecated */
	static readLocalFile(filePath) {
		// VERY SLOW! every single file you load with this costs a constant overhead of 70-80ms waiting for response.
		var result = null;
	...
	static linesCollisionPoint(v1, v2, v3, v4) {
		...
		if (s >= 0 && s <= 1 && t >= 0 && t <= 1) return new EngineLightweightPoint(v1.x + t * s1_x, v1.y + t * s1_y);
	...
	static distanceBetweenLines(l1, l2, l3, l4) {
		...
		return Math.sqrt(
			Math.min(
				...
				EngineUtils.distanceToLineSq(l2, l3, l4),
				EngineUtils.distanceToLineSq(l3, l1, l2),
				EngineUtils.distanceToLineSq(l4, l1, l2)
			)
		);
	}
	...
	static interpolate(val, min, max, interpType) {
		...
		switch (interpType) {
			...
			case EngineUtils.INTERPOLATE_IN_ELASTIC:
				return diff * ((0.04 - 0.04 / val) * Math.sin(25 * val) + 1) + min;
			case EngineUtils.INTERPOLATE_OUT_ELASTIC:
				return diff * (((0.04 * val) / --val) * Math.sin(25 * val)) + min;
			case EngineUtils.INTERPOLATE_SMOOTH_ELASTIC:
				...
				return (
					diff *
						((val -= 0.5) < 0
							? (0.02 + 0.01 / val) * Math.sin(50 * val)
							: (0.02 - 0.01 / val) * Math.sin(50 * val) + 1) +
					min
				);
			...
			case EngineUtils.INTERPOLATE_SMOOTH_BACK:
				var s = 1.70158;
				if ((val += val) < 1) return diff * ((1 / 2) * (val * val * (((s *= 1.525) + 1) * val - s))) + min;
			...
			case EngineUtils.INTERPOLATE_SMOOTH_EXPONENTIAL:
				if ((val += val) < 1) return diff * ((1 / 2) * Math.pow(2, 10 * (val - 1))) + min;
...
class EngineDebugUtils {
	...
	static drawPhysicsHitbox(graphics, inst) {
		if (!inst.__physicsObject) return;
